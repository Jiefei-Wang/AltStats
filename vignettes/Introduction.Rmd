...
To solve this issue, the package provides S3 and S4 classes to overload the default R functions. You can create an S3 or S4 object by calling `newAltWrapper`. 
```{r}
...
cumsum(C)
```
The variable `C` works as usual, most functions in `?"group generic"` has been overloaded to support ALTREP.
